...
SELECT
  ...
  (
    ...
    THEN pgp_sym_decrypt(zip_password, $1)
    ELSE ''
    END
...
type BackupsServiceGetBackupParams struct {
	EncryptionKey string
	ID            uuid.UUID
}

type BackupsServiceGetBackupRow struct {
	ID                   uuid.UUID
	DatabaseID           uuid.UUID
	DestinationID        uuid.NullUUID
	Name                 string
	...
	IsLocal              bool
	AllDatabases         bool
	DecryptedZipPassword string
}

// file: /mnt/a/__Github/pgbackweb/internal/service/backups/get_backup.sql
func (q *Queries) BackupsServiceGetBackup(ctx context.Context, arg BackupsServiceGetBackupParams) (BackupsServiceGetBackupRow, error) {
	row := q.db.QueryRowContext(ctx, backupsServiceGetBackup, arg.EncryptionKey, arg.ID)
	var i BackupsServiceGetBackupRow
	err := row.Scan(
		&i.ID,
		&i.DatabaseID,
		&i.DestinationID,
...
RETURNING id, name, bucket_name, access_key, secret_key, region, endpoint, created_at, updated_at, test_ok, test_error, last_test_at, provider, force_path_style
`
...
SELECT
  id, name, bucket_name, access_key, secret_key, region, endpoint, created_at, updated_at, test_ok, test_error, last_test_at, provider, force_path_style,
...
SET
  ...
  force_path_style = COALESCE($6, force_path_style),
  access_key = CASE
    WHEN $7::TEXT IS NOT NULL
    THEN pgp_sym_encrypt($7::TEXT, $8::TEXT)
    ELSE access_key
  END,
  secret_key = CASE
    WHEN $9::TEXT IS NOT NULL
    THEN pgp_sym_encrypt($9::TEXT, $8::TEXT)
    ELSE secret_key
  END
WHERE id = $10
RETURNING id, name, bucket_name, access_key, secret_key, region, endpoint, created_at, updated_at, test_ok, test_error, last_test_at, provider, force_path_style
...
type ExecutionsServiceGetBackupDataRow struct {
	...
	DestinationProvider               sql.NullString
